...
데이터유형
    ...
    ROWIDD 테이블에 있는 행의 고유한 주소를 나타내는 base-64
*/
...
제약조건 (Constraint)
    ...
    위에 테이블 생성 또는 변경시 설정하는 조건 (저장된 데이터의 신뢰성을 높이기 위해 사용)

    NOT NULL
        NULL로 입력이 되어서는 안되는 컬럼에 부여하는 조건으로
        컬럼 레벨에서만 부여할 수 있는 제약조건
    UNIQUE KEY(유일키)
        저장된 값이 중복되지 않고 오직 유일하게 유지되어야 할 때 사용하는 제약조건
        (NULL은 허용)
    PRIMARY KEY(대표키/기본키)
...
--제약 조건 추가하기
ALTER TABLE dept4 ADD(CONSTRAINT dept4_loc_ck CHECK(loc IN('1', '2')));